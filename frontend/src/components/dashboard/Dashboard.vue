            ...
            <v-row style="max-height: 600px">
                <v-col style="max-height: inherit;">
                    <v-row v-resize="updateStatsToDisplay">
                        ...
                        >
                            <StatCard
                                :loading="statCardComponentLoading"
                                v-bind:stat="stat"
                            />
                        </v-col>
                    </v-row>
                    <v-row>
                        <v-col>
                            <ReviewsCard
                                :loading="reviewsComponentLoading"
                                :reviews="reviews"
                            />
                        </v-col>
                    </v-row>
            ...
            <v-row>
                ...
                <v-row class="pl-4">
                    ...
                    >
                        <LocationCard
                            v-on:click.native="
                                changeCurrentSearch($event, place.name)
                            "
                            v-bind:place="place"
    ...
    methods: {
        updateStatsToDisplay() {
            ...
            for (let i = 0; i < numOfStatsToShow; i++) {
                newStatsToDisplay = Object.assign(
                    this.stats[i],
                    newStatsToDisplay
                );
            }
        ...
        updateData(places) {
            ...
            getWeather(places.location.lat, places.location.lng).then(
                (weatherDetails) => {
                    (this.stats[2].weather.value = weatherDetails.temperature),
                        (this.stats[2].weather.iconUrl =
                            weatherDetails.iconUrl),
                        (this.stats[2].weather.title =
            ...
            getCountryCode(places.location.lat, places.location.lng).then(
                (safetyRating) => {
                    this.stats[0].safetyRating.value =
