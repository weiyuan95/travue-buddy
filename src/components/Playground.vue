<template>
  <div>
    ...
    <hr>
    <div>
      <div v-for="location in bucketList" :key="location.name">
        {{ location.name }}
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    addToBucketList() {
      let location = {
        ...
        ytUrls: [{ title: "", url: "https://www.youtube.com/watch?v=xUhjFfFMo10" }],
        news: [
          {
    ...
    removeFromBucketList(locationName) {
      store.commit(BUCKET_REMOVE_LOCATION, { locationName });
    }
  }
};
</script>
